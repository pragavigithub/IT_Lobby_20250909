    ...
    if request.method == 'POST':
        try:
            ...
            if request.is_json:
                ...
                invoice.doc_date = datetime.strptime(invoice_date,
                                                     '%Y-%m-%d').date() if invoice_date else datetime.now().date()
    ...
    try:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                logging.info(
                    f" Invoice created successfully: DocEntry={invoice.sap_doc_entry}, DocNum={invoice.sap_doc_num}")
                ...
                if sap.base_url and sap.username and sap.password and sap.ensure_logged_in():
                    ...
                    if response.status_code == 200:
                        ...
                        if results:
                            ...
                            item_description = validation_data.get('itemName',
                                                                   validation_data.get('ItemName', item_description))
    ...
    try:
        ...
        if invoice.status not in ['draft', 'pending_qc']:
            return jsonify({'success': False,
                            'error': 'Cannot add items to finalized invoice. Document is locked after QC approval.'}), 400
        ...
        invoice_line.item_description = validation_result.get('itemName',
                                                              validation_result.get('ItemName', 'Unknown Item'))
        invoice_line.quantity = 1.0
        ...
        serial_item.item_description = validation_result.get('itemName',
                                                             validation_result.get('ItemName', 'Unknown Item'))
...
def remove_line_item(invoice_id, item_id):
    ...
    try:
        ...
        if invoice.status not in ['draft', 'pending_qc']:
            return jsonify({'success': False,
                            'error': 'Cannot remove items from finalized invoice. Document is locked after QC approval.'}), 400
...
def qc_approve_invoice(invoice_id):
    ...
    try:
        ...
        if sap_result['success']:
            ...
            logging.info(
                f" Invoice {invoice.id} approved and posted to SAP B1 (DocEntry: {sap_result.get('sap_doc_entry')})")
...
def qc_reject_invoice(invoice_id):
    ...
    try:
        ...
        logging.info(
            f" Invoice {invoice.id} rejected by QC and returned to draft for editing. Reason: {rejection_reason}")
...
def generate_sap_invoice_json(invoice):
    ...
    try:
        ...
        sap_invoice = {
            ...
            'BPL_IDAssignedToInvoice': getattr(first_serial, 'bpl_id', 5) if first_serial else 5,
